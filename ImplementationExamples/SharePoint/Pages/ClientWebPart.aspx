	...
	<head>
		...
		<script id="persontemplate" type="text/x-dot-template">
			<div class="card" id="p{{=it.id}}">
				{{?it.image}}<div style="background-image: url(http://your_acs_url/media/{{=it.image}});" class="imgdiv"></div>{{?}}
				<h2>{{=it.name}}</h2>
				<h4>{{=it.title}}</h4>
				<p>{{=it.phone}} {{? it.country }} <img src="http://your_acs_url/static/img/flags/{{=it.country}}.gif" width="16" height="11" alt="{{=it.country}}"/>{{?}}<br/>
					{{=it.mail}}</p>
				...
				<ul>
					{{~it.cv :cv:index}}
						{{? cv.status.percent > 10 }} 
							<li><a href="http://your_acs_url/cv/{{=cv.id}}/">{{=cv.tags}} {{=cv.last_edited}} {{=cv.status.percent}}%</a></li>
						{{?}}
					{{~}}
				</ul>
			</div>
